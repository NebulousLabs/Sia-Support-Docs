---
description: >-
  Learn step-by-step how to setup a new host on a Linux server
---

# Setup a New Host on Linux Server
This step-by-step guide will walk you through everything necessary to setup a new host on Ubuntu server using only the terminal.
...
First download the release using `wget`, extract the files using `unzip`, then move them to `/usr/local/bin`:
```sh
...
By creating a systemd unit we can start Sia whenever the server is booted instead of starting it manually. Run the following command to create a new systemd unit file:
```sh
...
## Forward ports
If you have a firewall or router you will need to forward/open TCP ports 9981-9983. There are tutorials available for most routers available on Google if you need help.
